"""
train_cfr_52_evbucket_oppbound.py
...
Information set:
    (player, turn, p1_draws, p2_draws, my_hand, ev_bucket, opp_bound)

where ev_bucket is the rounded expected one-card improvement from the player's
own perspective, using:
- their current hand
- their own discarded/dropped cards
- the opponent's visible discarded/dropped cards

The player does NOT observe the opponent's hidden hand.

Opponent bound idea:
- If the opponent discards value x, then regardless of whether x was the drawn
  card or the replaced card, all cards remaining in the opponent's hand are <= x.
- Therefore after seeing opponent discards, the acting player knows
      max(opponent hand) <= min(opponent discarded values).
- We store that scalar as opp_bound. Default is 10 before any opponent discard.
...
Output:
    cfr_52_evbucket_oppbound_policy.csv
"""
...
OUT_CSV = "cfr_52_evbucket_oppbound_policy.csv"
...
def opponent_bound_from_discards(opponent_discards: Tuple[int, ...]) -> int:
    """
    If opponent has visibly discarded x, then their current hand is all <= x.
    Multiple discards combine by taking the minimum discarded value.
    Before any opponent discard, the trivial bound is 10.
    """
...
def perceived_remaining_counts(
    ...
    """
    Player's perspective: full deck minus known cards.

    Known cards are:
    - my current hand
    - my own visible discards/drops
    - opponent's visible discards/drops

    This intentionally does NOT subtract the opponent's hidden hand.
    """
    hc = hand_counts(my_hand)
    rem = []
    for i in range(10):
        rem.append(max(0, FULL_DECK_COUNTS[i] - hc[i] - my_discards[i] - opponent_discards[i]))
    return tuple(rem)
...
def expected_one_draw_improvement(
    ...
    rem = perceived_remaining_counts(my_hand, my_discards, opponent_discards)
...
def ev_bucket(
    ...
    ev = expected_one_draw_improvement(my_hand, my_discards, opponent_discards)
...
def infoset_key(
    ...
    bucket = ev_bucket(my_hand, my_discards, opponent_discards)
    opp_bound = opponent_bound_from_discards(opponent_discards)
    return (player, turn, p1_draws, p2_draws, my_hand, bucket, opp_bound)
...
def cfr(
    ...
    opponent_discards = p2_discards if player == 1 else p1_discards

    key = infoset_key(player, turn, p1_draws, p2_draws, my_hand, my_discards, opponent_discards)
...
def write_policy_csv(path: str) -> None:
    ...
    for key, node in nodes.items():
        player, turn, p1_draws, p2_draws, hand, bucket, opp_bound = key
    ...
    rows.sort(key=lambda r: (r["player"], r["turn"], r["p1_draws"], r["p2_draws"], r["hand"], r["ev_bucket"], r["opp_bound"]))
